...
namespace conf {
	...
	const int INITFOOD = 1000; //initial number of full food cells
	...
	const float REPRATE=8; //amount of food required to be consumed for an agent to reproduce
	const float MAXMETABOLISM=3; //it is what it says. Metabolism is limited to [0,this]
    ...
    const float FOODWASTE= 0.0005; //how much food disapears if agent eats?
	...
	const float FOODSPREAD= 0.00005; //probability of a food cell seeding food to a nearby cell per tick
